...
from oci.key_management.models import GenerateKeyDetails, KeyShape, DecryptDataDetails
...
# 암호화 함수
async def encryption(data_to_encrypt_bytes: bytes, config, key_id, service_endpoint):
    start_time = time.time()
    
    # 비동기 클라이언트 생성
    crypto_client = KmsCryptoClient(config, service_endpoint=service_endpoint)
    
    generate_dek_details = GenerateKeyDetails(
        key_id=key_id,
        key_shape=KeyShape(
            algorithm="AES",
    ...
    generate_dek_response = await asyncio.to_thread(
        crypto_client.generate_data_encryption_key,
        generate_dek_details
    )
    
    dek_plaintext_base64 = generate_dek_response.data.plaintext
    dek_ciphertext = generate_dek_response.data.ciphertext
    dek_plaintext_bytes = base64.b64decode(dek_plaintext_base64)
    
    # 로컬 암호화 작업
    iv = os.urandom(16)
    cipher = Cipher(
        algorithms.AES(dek_plaintext_bytes),
        modes.GCM(iv),
...
# 복호화 함수
async def decryption(envelope_json: str, config, service_endpoint):
    start_time = time.time()
    
    # JSON 파싱
    envelope = json.loads(envelope_json)
    encrypted_data_b64 = envelope["encrypted_data"]
    dek_ciphertext = envelope["encrypted_dek"]
    key_id = envelope["key_id"]
    
    # 비동기 클라이언트 생성
    crypto_client = KmsCryptoClient(config, service_endpoint=service_endpoint)
    
    # DecryptDataDetails 객체 생성
    decrypt_details = DecryptDataDetails(
    ...
    dek_plaintext_base64 = decrypt_response.data.plaintext
    dek_plaintext_bytes = base64.b64decode(dek_plaintext_base64)
...
async def main():
    ...
    # 암호화
    envelope_json, encryption_time = await encryption(data_to_encrypt_bytes, config, key_id, service_endpoint)
    print(f"\n암호화 소요시간: {encryption_time:.3f}초")
    print(f"암호화된 데이터 크기: {len(envelope_json)} bytes")
    
    # 복호화
    decrypted_data, decryption_time = await decryption(envelope_json, config, service_endpoint)
